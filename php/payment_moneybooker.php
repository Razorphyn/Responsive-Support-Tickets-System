...
//0=>enabled,1=>merchant_id, 2=>payment_mail, 3=>currency, 4=>company, 5=>secret word
$adminmail=file('../php/config/setting.txt',FILE_IGNORE_NEW_LINES);

// Validate the Moneybookers signature
$concatFields = $_POST['merchant_id'].$_POST['transaction_id'].strtoupper(md5($moneybooker_setting[5])).$_POST['mb_amount'].$_POST['mb_currency'].$_POST['status'];

$headers = "MIME-Version: 1.0" . "\r\n";
$headers .= "Content-type:text/plain;charset=UTF-8" . "\r\n";
$headers .= 'From: '.$adminmail[1]."\r\n";

if (strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['pay_to_email'] == $moneybooker_setting[2] && $_POST['merchant_id']==$moneybooker_setting[1])
{
	if($file[0]==0){
	...
	if($amount==$_POST['mb_amount'] && $_POST['mb_currency']==$moneybooker_setting[3] && check_txnid($_POST['transaction_id'])){
